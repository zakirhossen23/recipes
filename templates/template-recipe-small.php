		...
		<?php if ( 'recipe' === get_post_type() ) : ?>
			<?php if ( mytheme_get_option( 'enable_ratings' ) ) : ?>
				...
				<li class="meta-percent">
					<svg class="rcps-icon rcps-icon-meta"><use xlink:href="<?php echo esc_url( get_template_directory_uri() ); ?>/images/icons.svg#icon-thumbs-up"/></svg><?php echo esc_html( $meta_percent ); ?>
				</li>
			<?php endif; ?>
			...
			<?php if ( is_numeric( $total_time ) && 0 < $total_time ) : ?>
				<li><svg class="rcps-icon rcps-icon-meta">
					<use xlink:href="<?php echo esc_url( get_template_directory_uri() ); ?>/images/icons.svg#icon-clock"/></svg><?php mytheme_minutes_to_hr_min( $total_time ); ?>
				</li>
			<?php endif; ?>

			<?php if ( get_post_meta( get_the_ID(), 'custom_meta_external_url', true ) && get_post_meta( get_the_ID(), 'custom_meta_external_site', true ) ) : ?>
				<li><svg class="rcps-icon rcps-icon-meta">
					<use xlink:href="<?php echo esc_url( get_template_directory_uri() ); ?>/images/icons.svg#icon-external-link-square"/></svg>
				...
				<?php if ( mytheme_get_option( 'link_external_recipes_to_source' ) ) : ?>
					<li>
						<a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php the_title(); ?>"><svg class="rcps-icon rcps-icon-meta"><use xlink:href="<?php echo esc_url( get_template_directory_uri() ); ?>/images/icons.svg#icon-link"/></svg><span class="screen-reader-text"><?php esc_html_e( 'Permalink', 'recipes' ); ?></span></a>
					</li>
				<?php endif; ?>
			<?php endif; ?>
